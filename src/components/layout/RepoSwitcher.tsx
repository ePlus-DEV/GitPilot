        ...
        >
          {repo ? (
            <>
              ...
              <ChevronDown size={11} className={`shrink-0 text-slate-500 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
            </>
          ) : (
            <>
              <span className="text-sm text-slate-400">Open repo</span>
      ...
      {isOpen && (
        <div className="absolute left-0 top-full z-50 mt-1 w-[280px] overflow-hidden rounded-lg border border-[#30363d] bg-[#161b22] shadow-2xl shadow-black/60">
          ...
          <div className="max-h-[300px] overflow-auto p-1">
            ...
            {filtered.length === 0 && (
              <div className="py-4 text-center text-xs text-slate-500">No repositories found</div>
            )}
          </div>
